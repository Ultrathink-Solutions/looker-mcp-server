...
class LookerUserIntrospector:
    ...
    Args:
        ...
        http_client: Optional pre-built ``httpx.AsyncClient`` (tests inject
            one; production lets the introspector lazily own a single reused
    ...
    async def verify(self, token: str) -> LookerUser:
        ...
        try:
            # Reuse a single AsyncClient across introspections — a fresh client
            # per token pays a new TCP+TLS handshake on every call. Lazily
            # created on first use (race-free under asyncio: no await between
            # the None-check and the assignment) and reused for the
            # introspector's lifetime; an injected client (tests) is reused
            # as-is. Never closed here — closing a shared client would break
            # subsequent calls; it is released on process shutdown.
            if self._client is None:
                self._client = httpx.AsyncClient(timeout=self._timeout, verify=self._verify_ssl)
            resp = await self._client.get(self._user_url, headers=headers, timeout=self._timeout)
