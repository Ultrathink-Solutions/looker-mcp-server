...
class OAuthIdentityProvider:
    ...
    async def resolve(self, context: RequestContext) -> LookerIdentity:
        ...
        if token and self._strip_bearer_scheme:
            ...
            if sep:
                # A scheme prefix is present: it MUST be a non-empty Bearer
                # token. A non-Bearer scheme (e.g. ``Basic abc``) or an empty
                # bearer (``Bearer ``) is a failed auth attempt — reject it
                # outright. Nulling the token instead would let a malformed
                # header silently fall through to API-key / service-account
                # fallback, downgrading a botched auth attempt to shared creds.
                rest = rest.strip()
                ...
                token = rest
            # A bare, schemeless token (no space) passes through unchanged —
            # the documented convenience for clients that omit ``Bearer ``.
        if token:
